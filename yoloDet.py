...
class YoloTRT:
    def __init__(self, library, engine, conf, yolo_ver):
        ...
        self.colors_mapping = {
            ...
            "person": (255, 0, 255),    # pink
            "bicycle": (0, 165, 255),   # orange
            "airplane": (255, 0, 255)   # purple
        }
    ...
    def non_max_suppression(self, result_boxes, result_scores, result_classid, iou_threshold=0.2, min_box_size=10, max_box_size=500):
      if result_boxes.size == 0:
        return np.array([]), np.array([]), np.array([])

     # Initialize lists for kept boxes, scores, and class IDs
      keep_boxes = []
      keep_scores = []
      keep_classid = []
    ...
    # Sort boxes by scores in descending order
      order = result_scores.argsort()[::-1]

      while order.size > 0:
        i = order[0]
        box_width = x2[i] - x1[i] + 1
        box_height = y2[i] - y1[i] + 1
        ...
        if min_box_size <= box_width <= max_box_size and min_box_size <= box_height <= max_box_size:
            keep_boxes.append(result_boxes[i])
            keep_scores.append(result_scores[i])
            keep_classid.append(result_classid[i])
        ...
        # Calculate width and height of intersections
      ...
      return np.array(keep_boxes), np.array(keep_scores), np.array(keep_classid)
    ...
    def Inference(self, img):
        ...
        t1 = time.time()
        self.context.execute_async(self.batch_size, bindings, stream.handle)
        ...
        for j in range(len(result_boxes)):
            ...
            det["box"] = box
            det_res.append(det)
            if det["conf"]>0.4 and (det["class"] in ["car", "motorcycle", "bus", "truck", "person", "bicycle","airplane"]):
    ...
    def PostProcess(self, output, origin_h, origin_w):
        ...
        result_boxes,result_scores,result_classid = self.non_max_suppression(result_boxes, result_scores, result_classid)
    ...
    def PlotBbox(self, x, img, label=None, line_thickness=1):
        tl = line_thickness  # line/font thickness
        c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))

        # If label is provided, extract class name and corresponding color
        if label:
            class_name = label.split(':')[0]
            color = self.colors_mapping.get(class_name, (255, 255, 255))  # default to white if class not found
            ...
            # If no label provided, use default color (white) to draw the rectangle
            color = (255, 255, 255)
            cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
